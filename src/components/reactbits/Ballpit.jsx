...
class Y extends c {
  constructor(params) {
    super({ vertexColors: true, ...params });
    this.onBeforeCompile = shader => {
      shader.vertexShader = `varying vec3 vBallNormal;\n${shader.vertexShader}`;
      shader.vertexShader = shader.vertexShader.replace(
        '#include <beginnormal_vertex>',
        '#include <beginnormal_vertex>\n  vBallNormal = normalize(normalMatrix * objectNormal);'
      );
      shader.fragmentShader = `varying vec3 vBallNormal;\n${shader.fragmentShader}`;
      shader.fragmentShader = shader.fragmentShader.replace(
        '#include <opaque_fragment>',
        `
          vec3 ballNormal = normalize(vBallNormal);
          vec3 highlightDirection = normalize(vec3(-0.55, 0.65, 0.9));
          float highlight = pow(max(dot(ballNormal, highlightDirection), 0.0), 4.0);
          float paleEdge = pow(1.0 - max(ballNormal.z, 0.0), 2.0);
          diffuseColor.rgb = mix(diffuseColor.rgb, vec3(1.0), highlight * 0.88);
          diffuseColor.rgb = mix(diffuseColor.rgb, vec3(0.93, 0.97, 1.0), paleEdge * 0.12);
          #include <opaque_fragment>
        `
      );
    };
...
const X = {
  count: 200,
  colors: [16777215, 14216447, 10013951, 3970815],
...
class Z extends d {
  constructor(e, t = {}) {
    const i = { ...X, ...t };
    const o = new g();
    const r = new Y({ color: 16777215, ...i.materialParams });
    super(o, r, i.count);
    this.config = i;
...
function createBallpit(e, t = {}) {
  ...
  i.renderer.toneMappingExposure = t.toneMappingExposure ?? 1;
